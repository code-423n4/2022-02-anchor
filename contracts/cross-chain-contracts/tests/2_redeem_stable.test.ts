...
// Tests the redeemStable method on the Ethereum Cross Anchor Bridge contract.
// In general, this test takes around 2-4 minutes to run.
describe("Integration / Redeem Stable (Redeem Tokens)", () => {
  ...
  it('Ethereum -> Terra', async function() {
    ...
    console.log("Redeem Stable (aUST)");
    let bridgeTransaction = await ethCrossAnchorBridge.redeemStable(
      CONSTANTS.eth_aust,
      1000
    );
    ...
    // Fetch sequences from the redeem receipt.
